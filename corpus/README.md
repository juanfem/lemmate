...
The Rust indexer is checked by `cargo test -p lemmate-core corpus`; the TypeScript one by
...
Regenerate an expectation after an intentional change, then review the diff by hand:
    ...
    cargo run -q -p lemmate-cli -- index corpus/NAME.md --json > corpus/NAME.json
